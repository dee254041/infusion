...
// This API route sets a server-side cookie to indicate bar authentication context
export async function GET(_request: NextRequest) {
  const response = NextResponse.json({ ok: true })
  
  // Set a cookie to indicate this is a bar user sign-in
  ...
  response.cookies.set('auth_context', 'bar', {
    path: '/',
    maxAge: 3600, // 1 hour
